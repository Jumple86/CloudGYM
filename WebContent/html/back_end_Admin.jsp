                ...
                <div class="main-out">
                    ...
                    <FORM METHOD="post" ACTION="admin.do" name="form1">
                        <ul>
                            ...
                            <li class="name-li">權限:
                                <span>
                                <input type="checkbox" name="commentAuth" value="${adminVO.commentAuth}"
                                ${adminVO.commentAuth == 1 ? "checked='true'" : ""} 
                                ${states == true ?  "" : 'disabled'}>討論區
                                </span>
                                <span>
                                <input type="checkbox" name="videoAuth" value="${adminVO.videoAuth}"
                                ${adminVO.videoAuth == 1 ? "checked='true'" : ""} 
                                ${states == true ? "" : 'disabled'}>影片審核
                                </span>
                                <span>
                                <input type="checkbox" name="subAuth" value="${adminVO.subAuth}"
                                ${adminVO.subAuth == 1 ? "checked='true'" : ""}
                                ${states == true ? "" : 'disabled'}>方案調整
                                </span>
                                <span>
                                <input type="checkbox" name="userAuth" value="${adminVO.userAuth}"
