...
//				coachVO.setUserID(userID);
				...
				if (!errorMsgs.isEmpty()) {
					...
					RequestDispatcher failureView = req.getRequestDispatcher("/coach/update_coach_input.jsp");
					failureView.forward(req, res);
					return; // 程式中斷
				}
				...
				/*************************** 2.開始修改資料 *****************************************/
				CoachService coachSvc = new CoachService();
				coachVO = coachSvc.updateCoach(userID, coachAccount, coachName, coachPassword, coachImg, userMobile,
		...
		if ("gotocoach".equals(action)) {
			...
			try {
				...
				RequestDispatcher successView = req.getRequestDispatcher("/html/coach_page.jsp");
				successView.forward(req, res);
			} catch (Exception e) {
