			...
			try {
				...
				if (!errorMsgs.isEmpty()) {
					RequestDispatcher failureView = req
							.getRequestDispatcher("/coach/select_page.jsp");
				...
				req.setAttribute("coachVO", coachVO); // 資料庫取出的empVO物件,存入req
...
//				coachVO.setUserID(userID);
				...
				// Send the use back to the form, if there were errors
				if (!errorMsgs.isEmpty()) {
					req.setAttribute("coachVO", coachVO); // 含有輸入格式錯誤的subscriptionVO物件,也存入req
					RequestDispatcher failureView = req
				...
				/***************************2.開始修改資料*****************************************/
				CoachService coachSvc = new CoachService();
				coachVO = coachSvc.updateCoach
						(userID, coachAccount, coachName, coachPassword, coachImg, userMobile, coachSex, coachBirthday, coachDescription, coachRegisteredDate, coachCertificate, reportedTimes);
				
				/***************************3.修改完成,準備轉交(Send the Success view)*************/
				req.setAttribute("coachVO", coachVO); // 資料庫update成功後,正確的的empVO物件,存入req
				String url = "/coach/listOneCoach.jsp";
        ...
        if ("insert".equals(action)) { // 來自addEmp.jsp的請求  
			...
			try {
				...
				/***************************2.開始新增資料***************************************/
				CoachService coachSvc = new CoachService();
				coachVO = coachSvc.addCoach
						(userID,coachAccount, coachName, coachPassword,coachImg1, userMobile, coachSex, coachBirthday,coachDescription,coachRegisteredDate,coachCertificate,reportedTimes);
        ...
        if ("delete".equals(action)) { // 來自listAllEmp.jsp
			...
			try {
				...
				String url = "/emp/listAllCoach.jsp";
